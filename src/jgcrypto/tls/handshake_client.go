...
func (hs *clientHandshakeState) processServerHello() (bool, error) {
	...
	if !clientDidNPN && serverHasNPN && false {
		c.sendAlert(alertHandshakeFailure)
...
func (hs *clientHandshakeState) sendFinished(out []byte) error {
	...
	c.offeredProtocols = hs.serverHello.nextProtos
	c.writeRecord(recordTypeChangeCipherSpec, []byte{1})
	if hs.serverHello.nextProtoNeg {
		nextProto := new(nextProtoMsg)
		proto, fallback := mutualProtocol(c.config.NextProtos,
			hs.serverHello.nextProtos)
...
func mutualProtocol(protos, preferenceProtos []string) (string, bool) {
	...
	// If there were no protos passed in (because they were not
	// configured) then pick the least preferred one from the server
	// offered protos
